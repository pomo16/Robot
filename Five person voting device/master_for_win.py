...
class Application_ui(Frame):
    ...
    def createWidgets(self):
        ...
        # 投票结果显示框
        self.SText = scrolledtext.ScrolledText(self.top, width=20, height=20, background='#ffffff',
                                               font=('宋体', 18, 'bold'))
        ...
        self.Combol1 = Combobox(self.top, text='', textvariable=self.Combol1Var, state="readonly",
                                values=self.Combol1List, font=('宋体', 12))
...
class Application(Application_ui):
    ...
    def Command1_Cmd(self, event=None):
        ...
        try:
            ...
            print(score)
            if score > 80:
                str1 = "选手" + self.Text2.get() + '  ' + str(score) + "分" + '  ' + "通过" + '  ' + time.strftime(
                    '%Y-%m-%d %H:%M:%S', time.localtime(time.time())) + '\n\n'
            else:
                str1 = "选手" + self.Text2.get() + '  ' + str(score) + "分" + '  ' + "不通过" + '  ' + time.strftime(
                    '%Y-%m-%d %H:%M:%S', time.localtime(time.time())) + '\n\n'
            self.SText.config(state=NORMAL)
